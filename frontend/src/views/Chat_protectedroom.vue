<template>
  <v-app >
    <v-container fluid>
      <div v-if="!created">
        <v-form @submit.prevent="submitIt(name, password, file)">
          ...
          <v-col cols="12" sm="6">
              <v-text-field clearable label="Name of the room"
                placeholder="name" v-model="name">
              </v-text-field>
              <v-text-field clearable label="Set a password" placeholder="Password"
                v-model="password">
              </v-text-field>
      ...
      <div v-else>
        <h2 class="textabovecenter">
          Protected channel {{ name }} was successfully created
        </h2>
  ...
  setup() {
    ...
    function submitIt(name: string, password: string, file: any) {
      if (verifyChannelName(name) === false) {
        return ;
      } else if (password.length > 10 || password.length < 4 || /^([a-zA-Z0-9-]+)$/.test(password) === false) {
        alert('Password must be between 4 to 10 letters and/or digits.');
        return ;
      }
      console.log(file);
      chatSocket.emit('createChannel', { name: name, password: password, type: ChannelType.PROTECTED, avatar: file});    
    }

    return { submitIt, name, password, file, previewFiles, created }
  }
})
</script>
