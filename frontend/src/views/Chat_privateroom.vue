    ...
    <v-container fluid>
      <div v-if="!created">
        <v-form @submit.prevent="submitIt(name, file)">
          ...
          <v-col cols="12" sm="6">
            <v-text-field clearable label="Name of the room" placeholder="name"
              v-model="name">
            </v-text-field>
...
export default defineComponent ({
  ...
  setup() {
    ...
    function submitIt(name: string, file: any) {
      ...
      chatSocket.emit('createChannel', { name: name, password: '', type: ChannelType.PRIVATE, avatar: file });   
    }

    return { submitIt, created, previewFiles, name, file }
