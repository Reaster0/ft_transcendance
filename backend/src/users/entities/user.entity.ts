...
export class User {
  ...
  @ApiProperty({
    type: Number,
    description:
      'An unique id number attributed to user inside database upon creation.',
  })
  id: number;
  ...
  @ApiProperty({
    type: String,
    description:
      'User private name. Cannot be modified and must only contains alphabetical characters.',
  })
  username: string;
  ...
  @ApiProperty({
    type: String,
    description:
      'User nickname. Can be modified and must only contains alphabetical characters.',
  })
  @IsAlphanumeric()
  nickname: string;
  ...
  @ApiProperty({
    type: String,
    description: 'User email. Must be under email format.',
  })
  @IsEmail()
  email: string;
  ...
  @ApiProperty({
    description: "If of avatar inside Avatar table, to retrieve user's avatar.",
  })
  ...
  @ApiProperty({
    type: Number,
    description: "Avatar id (inside avatar table) of user's avatar.",
  })
  avatarId?: number;
  ...
  @ApiProperty({
    type: String,
    description: 'User personal 2FA Secret (optional field)',
  })
  twoFASecret?: string;
  ...
  @ApiProperty({
    type: [Number],
    description: 'User friends, identified by unique ids inside an array.',
  })
  @IsNumber({}, { each: true })
  friends: number[];
  ...
  @ApiProperty({
    enum: Status,
    type: String,
    description: 'User status, either offline/online/playing.',
  })
  status: Status;
  ...
  @ApiProperty({
    type: Chan,
    description: 'Channel the user as joined/created',
  })
  ...
  @ApiProperty({
    type: Message,
    description: 'Message list the user as sended',
  })
  ...
  @ApiProperty({
    type: SocketJoined,
    description: 'Socket list for every channel the user is connected.',
  })
  ...
  @ApiProperty({
    type: Number,
    description:
      'Elo score, based on Elo chess system and modified after each match.',
  })
  eloScore: number;

  @ApiProperty({
    description:
      'History of games won in relation with corresponding gameHistory entity.',
  })
  ...
  @ApiProperty({
    description:
      'History of games lost in relation with corresponding gameHistory entity.',
  })
  ...
  async encryptSecret() {
    ...
    const iv = Buffer.from(
      crypto.randomBytes(parseInt(process.env.ENCRYPTION_IV_LENGTH)),
    )
      .toString('hex')
    ...
    const encrypted = Buffer.concat([
      cipher.update(this.twoFASecret),
      cipher.final(),
    ]);
  ...
  decryptSecret() {
    ...
    const parts = this.twoFASecret.includes(':')
      ? this.twoFASecret.split(':')
      : [];
