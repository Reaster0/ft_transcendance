...
export class UsersController {
  ...
  logged(@Req() req: RequestUser): boolean {
    this.logger.log(
      "Get('logged') route called by user " + req.user.username + ' (username)',
    );
  ...
  currentUser(@Req() req: RequestUser): Promise<Partial<User>> {
    this.logger.log(
      "Get('currentUser') route called by user " +
        req.user.username +
        ' (username)',
    );
    const user: User = req.user;
  ...
  userInfo(@Query('username') username: string): Promise<Partial<User>> {
    this.logger.log(
      "Get('userInfo') route called for user " + username + ' (username)',
    );
  ...
  getPartialUserInfo(@Query('userId') userId: string): Promise<Partial<User>> {
    this.logger.log(
      "Get('partialInfo') route called for user " + userId + ' (user id)',
    );
  ...
  @ApiCreatedResponse({
    description: 'The user has been successfully retrieved or registered.',
    type: User,
  })
  @ApiBadRequestResponse()
  /** End of swagger **/
  retrieveOrCreateUser(@Body() createUserDto: CreateUserDto): Promise<User> {
    this.logger.log(
      "Post('getOrRegister') route called for user " +
  ...
  uploadAvatar(
    ...
    this.logger.log(
      "Post('uploadAvatar') route called by user " +
        req.user.username +
        ' (username)',
    );
  ...
  async updateUser(
    ...
    this.logger.log(
      "Post('settings') route called by user " +
        req.user.username +
        ' (username)',
    );
  ...
  async loginUser(@Req() req: RequestUser) {
    this.logger.log(
      "Patch('login') route called by user " +
        req.user.username +
        ' (username)',
    );
  ...
  async logoutUser(
    ...
    this.logger.log(
      "Patch('logout') route called by user " +
        req.user.username +
        ' (username)',
    );
  ...
  async removeUser(
    ...
    this.logger.log(
      "Delete('deleteAccount') route called by user " +
        req.user.username +
        ' (username)',
    );
